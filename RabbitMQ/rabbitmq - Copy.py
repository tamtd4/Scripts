...
class RabbitMQAPI(object):
    ...
    def __init__(self, user_name='guest', password='guest', host_name='',
                 port=15672, conf='C:/zabbix/conf/zabbix_agentd.conf', senderhostname=None, protocol='http'):
        self.user_name = user_name
        self.password = password
        self.host_name = host_name or socket.gethostname()
        self.port = port
        self.conf = conf or 'C:/zabbix/conf/zabbix_agentd.conf'
    ...
    def call_api(self, path):
        ...
        url = '{0}://{1}:{2}/api/{3}'.format(self.protocol, "localhost", self.port, path)
    ...
    def _send_data(self, tmpfile):
        ...
        args = 'C:/zabbix/bin/win32/zabbix_sender.exe -z 172.20.19.100 -p 10051 -vv -c {0} -i {1}'
...
def main():
    ...
    parser.add_option('--conf', default='C:/zabbix/conf/zabbix_agentd.conf')
    parser.add_option('--senderhostname', default='', help='Allows including a sender parameter on calls to zabbix_sender')
    parser.add_option('--logfile', help='File to log errors (defaults to C:/rabbitmq_zabbix.log)', default='C:/rabbitmq_zabbix.log')
    ...
    logging.basicConfig(filename=options.logfile or "C:/rabbitmq_zabbix.log", level=logging.getLevelName(options.loglevel or "INFO"), format='%(asctime)s %(levelname)s: %(message)s')
    ...
    if options.check == 'list_queues':
        ...
        print api.check_aliveness(options.metric)
